...
# Work out de/dt
scc_balti_lac$de.dt <- NA
i <-"Los Angeles County"
for (i in unique(scc_balti_lac$county)){
    stemp <- scc_balti_lac$emissions[scc_balti_lac$year == 1999 & scc_balti_lac$county == i]
    scc_balti_lac$de.dt[scc_balti_lac$county == i] <- scc_balti_lac$emissions[scc_balti_lac$county == i] - stemp
    }
scc_balti_lac$de.dt <- abs(scc_balti_lac$de.dt)
...
p <- ggplot(scc_balti_lac, aes(x=year, y=de.dt, col=colref))
