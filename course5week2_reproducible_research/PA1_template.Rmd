...
The total number of steps taken during the sampling period was `r tot.steps`.
...
```
The mean number of steps taken during the sampling period was `r mean.steps` while the median was `r median.steps`.
...
The highest mean number of steps were taken during interval `r max.int`.
...
```
There are `r num.na` missing values (coded as NA) in the dataset.
...
After replacing missing step number data with median values for the interval, the mean number of steps taken during the sampling period was `r mean.steps.adj` while the median was `r median.steps.adj`. the mean and median numbers of steps changed by `r diff(c(mean.steps, mean.steps.adj))` and `r diff(c(median.steps, median.steps.adj))` respectively.
